function handles=sic_spm8_dicom_convert(handles)
% function sang_spm2_dicom_convert(hdr,opts)
...
[handles.b.current_volume.images,handles.b.current_volume.guff]     = select_tomographic_images(handles.b.current_volume.hdr);
[handles.b.current_volume.mosaic,handles.b.current_volume.standard] = select_mosaic_images(handles.b.current_volume.images);

 
 
 
 
if (strcmp(opts,'all') | strcmp(opts,'mosaic')) & ~isempty(handles.b.current_volume.mosaic),
	handles=convert_mosaic(handles); 
end;
if (strcmp(opts,'all') | strcmp(opts,'standard')) & ~isempty(handles.b.current_volume.standard),
	handles=convert_standard(handles);
end;
return;
...
%_______________________________________________________________________
function handles=convert_mosaic(handles)
% spm_progress_bar('Init',length(hdr),['Writing Mosaic'], 'Files written');

 

for i=1:length(handles.b.current_volume.mosaic),
...
% 		hdr{i}.SeriesNumber, hdr{i}.AcquisitionNumber, hdr{i}.InstanceNumber);
    ...
    handles.b.current_volume.fname = [handles.b.conf.out_volume_path filesep sprintf('f_%s-%04d-%05d-%06d.nii', strip_unwanted(handles.b.conf.sub_name),...
	handles.b.current_volume.mosaic{i}.SeriesNumber, handles.b.current_volume.mosaic{i}.AcquisitionNumber,handles.b.current_volume.mosaic{i}.InstanceNumber)];

    handles.b.current_volume.mosaicname = [handles.b.conf.out_volume_path filesep sprintf('%s-%04d-%05d-%06d.nii', strip_unwanted(handles.b.conf.sub_name),...
	handles.b.current_volume.mosaic{i}.SeriesNumber, handles.b.current_volume.mosaic{i}.AcquisitionNumber,handles.b.current_volume.mosaic{i}.InstanceNumber)];


	% Image dimensions and data
	%-------------------------------------------------------------------
	nc = handles.b.current_volume.mosaic{i}.Columns;
	nr = handles.b.current_volume.mosaic{i}.Rows;
	np = read_AcquisitionMatrixText(handles.b.current_volume.mosaic{i});

	if rem(nc, np(1)) | rem(nr, np(2)),
		warning(sprintf('%s: %dx%d wont fit into %dx%d.',handles.b.current_volume.mosaic{i}.Filename,...
			np(1), np(2), nc,nr));
		return;
	end;
	dim    = [np read_NumberOfImagesInMosaic(handles.b.current_volume.mosaic{i})];

	mosaic = read_image_data(handles.b.current_volume.mosaic{i});
	volume = zeros(dim);
	for j=1:dim(3),
	...
	vox    = [handles.b.current_volume.mosaic{i}.PixelSpacing;handles.b.current_volume.mosaic{i}.SpacingBetweenSlices];
	pos    = handles.b.current_volume.mosaic{i}.ImagePositionPatient;
	orient = reshape(handles.b.current_volume.mosaic{i}.ImageOrientationPatient,[3 2]);
	...
	%-------------------------------------------------------------------
	SliceNormalVector = read_SliceNormalVector(handles.b.current_volume.mosaic{i});
	if det([reshape(handles.b.current_volume.mosaic{i}.ImageOrientationPatient,[3 2]) SliceNormalVector(:)])<0;
	...
	%-------------------------------------------------------------------
	tim = datevec(handles.b.current_volume.mosaic{i}.AcquisitionTime/(24*60*60));
	descrip = sprintf('%gT %s %s TR=%gms/TE=%gms/FA=%gdeg %s %d:%d:%.5g Mosaic',...
		handles.b.current_volume.mosaic{i}.MagneticFieldStrength, handles.b.current_volume.mosaic{i}.MRAcquisitionType,...
		deblank(handles.b.current_volume.mosaic{i}.ScanningSequence),...
		handles.b.current_volume.mosaic{i}.RepetitionTime,handles.b.current_volume.mosaic{i}.EchoTime,handles.b.current_volume.mosaic{i}.FlipAngle,...
		datestr(handles.b.current_volume.mosaic{i}.AcquisitionDate),tim(4),tim(5),tim(6));
	...
	if isfield(handles.b.current_volume.mosaic{i},'RescaleSlope') & handles.b.current_volume.mosaic{i}.RescaleSlope ~= 1,
		volume = volume*handles.b.current_volume.mosaic{i}.RescaleSlope;
	end;
	if isfield(handles.b.current_volume.mosaic{i},'RescaleIntercept') & handles.b.current_volume.mosaic{i}.RescaleIntercept ~= 0,
		volume = volume + handles.b.current_volume.mosaic{i}.RescaleIntercept;
	end;

	V = struct('fname',handles.b.current_volume.fname,'dim',dim,'mat',mat,'descrip',descrip,'dt',[sqrt(handles.b.current_volume.mosaic{1, 1}.BitsAllocated) 0] );
	handles.b.current_volume.V = spm_write_vol(V,volume);
    handles.b.current_volume.volume=volume;
...
%_______________________________________________________________________
function handles=convert_standard(handles)
handles.b.current_volume.standard = sort_into_volumes(handles.b.current_volume.standard);
for i=1:length(handles.b.current_volume.standard),
	handles=write_volume(handles.b.current_volume.standard{i},handles);
end;
return;
%_______________________________________________________________________
...
%_______________________________________________________________________
function handles=write_volume(hdr,handles)
...
handles.b.current_volume.fname = [handles.b.conf.out_volume_path filesep sprintf('s_%s-%.4d-%.5d-%.6d.nii', strip_unwanted(handles.b.conf.sub_name),...
	hdr{1}.SeriesNumber, hdr{1}.AcquisitionNumber, hdr{1}.InstanceNumber)];

handles.b.current_volume.name = [handles.b.conf.out_volume_path filesep sprintf('%s-%04d-%05d-%06d.nii', strip_unwanted(handles.b.conf.sub_name),...
	hdr{1}.SeriesNumber, hdr{1}.AcquisitionNumber, hdr{1}.InstanceNumber)];
...
aa=orient*diag(vox); aa(1:3,4)= pos;
dicom_to_patient = [aa ; 0 0 0 1];
patient_to_tal   = diag([-1 -1 1 1]);
mat              = patient_to_tal*dicom_to_patient*analyze_to_dicom;
...
V = struct('fname',handles.b.current_volume.fname, 'dim',dim, 'pinfo',pinfo, 'mat',mat, 'descrip',descrip);
V = spm_create_vol(V);
for i=1:length(hdr),
