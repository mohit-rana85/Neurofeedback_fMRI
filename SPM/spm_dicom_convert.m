function handles = spm_dicom_convert(handles)%(hdr,opts,root_dir,format,out_dir,meta)
...
if strcmp(handles.b.session_type,'Anatomical Run')
    out_dir  =  handles.b.conf.anatomy_path; 
elseif  strcmp(handles.b.session_type,'Dummy Functional Run')
   out_dir  =   handles.b.conf.dummy_path;
elseif strcmp(handles.b.session_type,'Localizer Collect Data')
    out_dir= handles.b.conf.localizer_path;
else
    out_dir  =  handles.b.conf.feed_path;
end
meta     = false;  
hdr=handles.b.current_volume.hdr;
%-Select files
%--------------------------------------------------------------------------
...
if (strcmp(opts,'all') || strcmp(opts,'mosaic')) && ~isempty(mosaic)
    [handles.b.current_volume.fname,handles] = convert_mosaic(mosaic,root_dir,format,out_dir,meta,handles);
end
if (strcmp(opts,'all') || strcmp(opts,'standard')) && ~isempty(standard)
   handles.b.current_volume.fname= convert_standard(standard,root_dir,format,out_dir,meta);
end
...
%==========================================================================
function [fnames,handles] = convert_mosaic(hdr,root_dir,format,out_dir,meta,handles)
...
for i=1:length(hdr)
    ...
    handles.b.current_volume.fname = [out_dir filesep sprintf('f_%s-%04d-%05d-%06d.nii', strip_unwanted(handles.b.conf.sub_name),...
	handles.b.current_volume.hdr{i}.SeriesNumber, handles.b.current_volume.hdr{i}.AcquisitionNumber,handles.b.current_volume.hdr{i}.InstanceNumber)];
    fnames{i} =handles.b.current_volume.fname;% getfilelocation(hdr{i},root_dir,'f',format,out_dir);
    handles.b.current_volume.mosaicname = [out_dir filesep sprintf('%s-%04d-%05d-%06d.nii', strip_unwanted(handles.b.conf.sub_name),...
	handles.b.current_volume.hdr{i}.SeriesNumber, handles.b.current_volume.hdr{i}.AcquisitionNumber,handles.b.current_volume.hdr{i}.InstanceNumber)];
    ...
    if (handles.b.conf.session>1 && isfield(handles.b.Feedback,'feed_idx')) || (handles.b.conf.session==1 && handles.b.Feedback.dummy_idx==1)
         if isfield(handles.b.pre,'current_M')
              mat=handles.b.pre.current_M\mat;
         end
    end
    ...
    dat(:,:,:) = volume;
    V = struct('fname',fnames{i},'dim',dim,'dt',dt,'mat',mat,'descrip',descrip);
