...
%                undefined.
   ...
   %#H = specify_image(img,h)
%#addblobs(handle, xyz, t, mat, name)
%#addimage(handle, fname)
%#addcolouredblobs(handle, xyz, t, mat, colour, name)
%#addcolouredimage(handle, fname,colour)
%#addtruecolourimage(handle,fname,colourmap,prop,mx,mn)
%#addcolourbar(vh,bh)
%#rmblobs(handle)
%#register(hreg)
%#callback
...
%#space(handle,M,dim)
%#zoom_op(fov,res)
%#repos_start(varargin)
%#repos_move(varargin)
%#repos_end(varargin)
%#scroll_wheel(varargin)
%#bbox(handles)
%#mx = maxval(vol)
%#mn = minval(vol)
%#redraw(arg1)
%#redraw_all
%#redraw_colourbar(vh,bh,interval,cdata)
%#centre = findcent
%#handles = valid_handles(handles)
%#reset_st
%#img = scaletocmap(inpimg,mn,mx,cmap,miscol)
%#cmap = getcmap(acmapname)
%#item_parent = addcontext(volhandle)
%#addcontexts(handles)
%#rmcontexts(handles)
%#c_menu(varargin)
