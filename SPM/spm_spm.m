...
%--------------------------------------------------------------------------
VM = struct(...
    'fname',   [pth, filesep, 'mask' file_ext],...
    'dim',     DIM,...
...
for i = 1:nBeta
    Vbeta(i).fname   = [pth, filesep,sprintf('beta_%04d',i) file_ext];
...
%--------------------------------------------------------------------------
VResMS = struct(...
    'fname',   [pth, filesep,'ResMS' file_ext],...
    'dim',     DIM,...
...
for i = 1:nSres
    VResI(i).fname   = [pth, filesep,sprintf('ResI_%04d', i) file_ext];
...
if ~spm_mesh_detect(VY)
    [FWHM,VRpv,R] = spm_est_smoothness(VResI,VM,[nScan erdf],pth);
else
    VRpv = struct(...
        'fname',   [pth, filesep,'RPV' file_ext],...
        'dim',     DIM,...
        'dt',      [spm_type('float64') spm_platform('bigend')],...
        'mat',     M,...
        'pinfo',   [1 0 0]',...
...
save([pth,filesep,'SPM.mat'],'SPM', spm_get_defaults('mat.format'));
