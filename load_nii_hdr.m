...
function [hdr, filetype, fileprefix, machine] = load_nii_hdr(fileprefix)
   ...
   if ~isempty( strfind(fileprefix,'.nii'))
      new_ext = 1;
      fileprefix1 = strrep(fileprefix,'.nii','');
      if iscell( fileprefix1)
         fileprefix=fileprefix1{1};
      else
          fileprefix=fileprefix1
      end
   end

   if ~isempty( strfind(fileprefix,'.hdr'))%findstr('.hdr',fileprefix)
      fileprefix1 = strrep(fileprefix,'.hdr','');
         if iscell( fileprefix1)
         fileprefix=fileprefix1{1};
      else
          fileprefix=fileprefix1
   ...
   if ~isempty( strfind(fileprefix,'.img'))
      fileprefix1 = strrep(fileprefix,'.img','');
        if iscell( fileprefix1)
         fileprefix=fileprefix1{1};
      else
          fileprefix=fileprefix1
