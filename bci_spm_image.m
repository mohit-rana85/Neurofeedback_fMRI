function bci_spm_image(op,varargin)
...
%     spm_help('!ContextHelp',[mfilename,'.m']);
    ...
    %-----------------------------------------------------------------------
    P      = spm_select(1,'image','Select anatomical (T1) image',[],0);
    WP     = spm_select(1,'image','Select normalised anatomical (wT1) image',[], 0);
    WP     = spm_vol(WP);
    
    wT1_info = WP;
    bci_spm_image('init',P);
    st.registration_type = 'NIRS_MRI';
    return;
end;
...
if strcmp(op,'standalone');
   template_fname= spm_select(1,'image','Select SPM-template',[],0);
   bci_spm_image('init', template_fname);   
   st.registration_type = 'standalone';
   return;
end
if strcmp(op,'repos'),
    ...
    % spm_orthviews('MaxBB');
    bci_spm_image('zoom_in');
    bci_spm_image('update_info');
...
if strcmp(op,'addindiloc'),
    
%    global h_chanloc;
%    global handles_chanloc;
%    st.count = st.count + 1;
%    st.pos{st.count} = num2str(st.cur_pos);
    pos=spm_orthviews('pos'); st.count=st.count+1; WS = spm('WinScale');
    pos=(round(pos*10))/10;
    %fg = spm_figure('Findwin','Graphics');
    st.handles_roi(st.count)=uicontrol(st.fg,'Style','Text', 'Position',[365  570-20*st.count 170 016].*WS,'String',['ROI ', num2str(st.count), ': ', num2str(pos')]);
    st.MNI_coordinates(st.count,:)=pos';
%    switch st.registration_type
%        case 'standalone'
%            global handles_standalone;
%            [temp1 temp2] = NIRS_Registration_Standalone;
%            handles_standalone{1} = temp1;
%            handles_standalone{2} = temp2;           
%            NIRS_Registration_Standalone('NIRS_Registration_Standalone_OpeningFcn', handles_standalone{1}, 1, handles_standalone{2}, {}, st.cur_pos);
%        case 'NIRS_MRI'
...
%            NIRS_Import_Indicator_Loc('NIRS_Import_Indicator_Loc_OpeningFcn',handles_registration_NIRS_MRI{1},1,handles_registration_NIRS_MRI{2},{},st.cur_pos);
%    end    
end

if strcmp(op,'delindiloc')
    if st.count>0
        delete(st.handles_roi(st.count))
        st.MNI_coordinates(st.count,:)=[];
        st.count=st.count-1;
    end
    %     if st.count < 1
    %         return
...
%             handles_standalone{1} = temp1;
%             handles_standalone{2} = temp2;
%             NIRS_Registration_Standalone('NIRS_Registration_Standalone_OpeningFcn',handles_standalone{1} ,2, handles_standalone{2},{},st.cur_pos);
%         case 'NIRS_MRI'
%             global handles_registration_NIRS_MRI;
%             [temp1 temp2] = NIRS_Import_Indicator_Loc;
%             handles_registration_NIRS_MRI{1} = temp1;
%             handles_registration_NIRS_MRI{2} = temp2;
%             NIRS_Import_Indicator_Loc('NIRS_Import_Indicator_Loc_OpeningFcn',handles_registration_NIRS_MRI{1},2,handles_registration_NIRS_MRI{2},{},st.cur_pos);
%     end
end
...
if strcmp(op,'addblobs'),
    ...
    for i=1:nblobs,
        ...
        set(st.blobber,'String','Remove Blobs','Callback','bci_spm_image(''rmblobs'');');
    end;
...
if strcmp(op,'rmblobs'),
    ...
    set(st.blobber,'String','Add Blobs','Callback','bci_spm_image(''addblobs'');');
end;
...
%         'Images Complete');
    ...
    if sum((tmp(:)-st.vols{1}.mat(:)).^2) > 1e-8,
        bci_spm_image('init',st.vols{1}.fname);
    end;
    return;
end;
...
if strcmp(op,'Done'),
     fid= fopen([st.outdir, filesep,'MNI_coordinate.txt'],'wt');
     for iin=1:size(st.MNI_coordinates,1)
         fprintf(fid,'%s\n',num2str(st.MNI_coordinates(iin,:)));
     end
    fclose(fid);
    delete(st.fg)
end
...
if strcmp(op,'init'),
    ...
    spm_orthviews('Redraw');
    st.callback = 'bci_spm_image(''shopos'');';
    ...
    %-----------------------------------------------------------------------
    uicontrol(fg,'Style','Frame','Position',[60 25 200 325].*WS,'DeleteFcn','bci_spm_image(''reset'');');
    ...
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',1)','Position',[175 220 065 020].*WS,'String','0','ToolTipString','translate');
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',2)','Position',[175 200 065 020].*WS,'String','0','ToolTipString','translate');
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',3)','Position',[175 180 065 020].*WS,'String','0','ToolTipString','translate');
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',4)','Position',[175 160 065 020].*WS,'String','0','ToolTipString','rotate');
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',5)','Position',[175 140 065 020].*WS,'String','0','ToolTipString','rotate');
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',6)','Position',[175 120 065 020].*WS,'String','0','ToolTipString','rotate');
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',7)','Position',[175 100 065 020].*WS,'String','1','ToolTipString','zoom');
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',8)','Position',[175  80 065 020].*WS,'String','1','ToolTipString','zoom');
    uicontrol(fg,'Style','edit','Callback','bci_spm_image(''repos'',9)','Position',[175  60 065 020].*WS,'String','1','ToolTipString','zoom');

    uicontrol(fg,'Style','Pushbutton','String','Reorient images...','Callback','bci_spm_image(''reorient'')',...
        'Position',[70 35 125 020].*WS,'ToolTipString','modify position information of selected images');

    uicontrol(fg,'Style','Pushbutton','String','Reset...','Callback','bci_spm_image(''resetorient'')',...
        'Position',[195 35 55 020].*WS,'ToolTipString','reset orientations of selected images');
    
    uicontrol(fg,'Style','Pushbutton','String','+','FontSize',13,'Callback','bci_spm_image(''addindiloc'')',...
        'Position',[265 290 35 020].*WS);
    uicontrol(fg,'Style','Pushbutton','String','-','FontSize',15,'Callback','bci_spm_image(''delindiloc'')',...
        'Position',[265 270 35 020].*WS);
    uicontrol(fg,'Style','Pushbutton','String','Done','FontSize',11,'Callback','bci_spm_image(''Done'')',...
        'Position',[265 245 35 020].*WS); 
    uicontrol(fg,'Style','Frame','Position',[360  390 225 205].*WS);
    uicontrol(fg,'Style','Text', 'Position',[395  570 170 016].*WS,'String','Selected MNI for ROIs');
    uicontrol(fg,'Style','Text', 'Position',[15   710 30 30].*WS,'String','L','fontsize',20);
    uicontrol(fg,'Style','Text', 'Position',[555  710 30 30].*WS,'String','R','fontsize',20);
    % Crosshair position
    %-----------------------------------------------------------------------
    ...
    st.mp = uicontrol(fg,'Style','edit', 'Position',[110 295 135 020].*WS,'String','','Callback','bci_spm_image(''setposmm'')','ToolTipString','move crosshairs to mm coordinates');
    st.vp = uicontrol(fg,'Style','edit', 'Position',[110 275 135 020].*WS,'String','','Callback','bci_spm_image(''setposvx'')','ToolTipString','move crosshairs to voxel coordinates');
    ...
    st.zoomer = uicontrol(fg,'Style','popupmenu' ,'Position',[315 75 125 20].*WS,...
        ...
        'Callback','bci_spm_image(''zoom_in'')','ToolTipString','zoom in by different amounts');
    c = 'if get(gco,''Value'')==1, spm_orthviews(''Space''), else, spm_orthviews(''Space'', 1);end;bci_spm_image(''zoom_in'')';
    ...
    st.win = uicontrol(fg,'Style','popupmenu','Position',[315 35 125 20].*WS,...
        'String',char('Auto Window','Manual Window'),'Callback','bci_spm_image(''window'');','ToolTipString','range of voxel intensities displayed');
    % uicontrol(fg,'Style','pushbutton','Position',[315 35 125 20].*WS,...
    % 	'String','Window','Callback','bci_spm_image(''window'');','ToolTipString','range of voxel intensities % displayed');
    st.blobber = uicontrol(fg,'Style','pushbutton','Position',[450 35 125 20].*WS,...
        'String','Add Blobs','Callback','bci_spm_image(''addblobs'');','ToolTipString','superimpose activations');
    st.fg = fg;st.count=0;
    [aa,~,~]=fileparts(P.fname); a1=strfind(aa, filesep);
    st.outdir=[ aa(1:a1(end)),'Anatomyf',filesep,'Original'];
end;
